...
exports.enqueue = function (res, next) {
  async.map(res._enqueued, function (item, next) {
    makeScript(item, next);
  },
  function (err, enqueued) {
    if(enqueued) {
      //## Option to push to foot instead
      res.app.locals._head.push(enqueued.join(''));
    }
